...
def CostofItem():
    ...
    if var1.get() != 0 or var2.get() != 0 or var3.get() != 0 or var4.get() != 0 or var5.get() != 0 or var6.get() != 0 or var7.get() != 0 \
        ...
        subtotalOfItems = 'Rs', str(
            '%.2f' % (priceofcakes + priceofdrinks + priceoffood))
        subtotal.set(subtotalOfItems)

        tc = 'Rs', str('%.2f' %
                       (priceofcakes + priceofdrinks + priceoffood + 15.5))
        totalcost.set(tc)
...
def receipt():
    ...
    if var1.get() != 0 or var2.get() != 0 or var3.get() != 0 or var4.get() != 0 or var5.get() != 0 or var6.get() != 0 or var7.get() != 0 \
        ...
        if e_daal.get() != '0':
            textReceipt.insert(
                END, f'Daal:\t\t\t{float(e_daal.get()) * 45}\n\n')

        if e_roti.get() != '0':
            textReceipt.insert(
                END, f'Roti:\t\t\t{float(e_roti.get()) * 10}\n\n')

        if e_chawal.get() != '0':
            textReceipt.insert(
                END, f'Chawal:\t\t\t{float(e_chawal.get()) * 30}\n\n')

        if e_sabji.get() != '0':
            textReceipt.insert(
                END, f'Sabji:\t\t\t{float(e_sabji.get()) * 50}\n\n')

        if e_fish.get() != '0':
            textReceipt.insert(
                END, f'Fish:\t\t\t{float(e_fish.get()) * 120}\n\n')

        if e_paneer.get() != '0':
            textReceipt.insert(
                END, f'Paneer:\t\t\t{float(e_paneer.get()) * 110}\n\n')

        if e_kebab.get() != '0':
            textReceipt.insert(
                END, f'Kabab:\t\t\t{float(e_kebab.get()) * 100}\n\n')

        if e_chicken.get() != '0':
            textReceipt.insert(
                END, f'Chicken:\t\t\t{float(e_chicken.get()) * 140}\n\n')

        if e_mutton.get() != '0':
            textReceipt.insert(
                END, f'Mutton:\t\t\t{float(e_mutton.get()) * 160}\n\n')

        if e_lassi.get() != '0':
            textReceipt.insert(
                END, f'Lassi:\t\t\t{float(e_lassi.get()) * 40}\n\n')
        if e_coffee.get() != '0':
            textReceipt.insert(
                END, f'Coffee:\t\t\t{float(e_coffee.get()) * 40}\n\n')
        if e_faluda.get() != '0':
            textReceipt.insert(
                END, f'Faluda:\t\t\t{float(e_faluda.get()) * 50}\n\n')
        if e_shikanji.get() != '0':
            textReceipt.insert(
                END, f'Shikanji:\t\t\t{float(e_shikanji.get()) * 30}\n\n')
        if e_jaljeera.get() != '0':
            textReceipt.insert(
                END, f'Jal-jeera:\t\t\t{float(e_jaljeera.get()) * 20}\n\n')
        if e_rohhafza.get() != '0':
            textReceipt.insert(
                END, f'Rooh Afza:\t\t\t{float(e_rohhafza.get()) * 30}\n\n')
        if e_masalachai.get() != '0':
            textReceipt.insert(
                END, f'Masala Chai:\t\t\t{float(e_masalachai.get()) * 20}\n\n')
        if e_badammilk.get() != '0':
            textReceipt.insert(
                END, f'Badam Milk:\t\t\t{float(e_badammilk.get()) * 50}\n\n')
        if e_coldrinks.get() != '0':
            textReceipt.insert(
                END, f'Cold Drink:\t\t\t{float(e_coldrinks.get()) * 60}\n\n')

        if e_kikat.get() != '0':
            textReceipt.insert(
                END, f'Kitkat Cake:\t\t\t{float(e_kikat.get()) * 480}\n\n')
        if e_apple.get() != '0':
            textReceipt.insert(
                END, f'Apple Cake:\t\t\t{float(e_apple.get()) * 350}\n\n')
        if e_vanilla.get() != '0':
            textReceipt.insert(
                END, f'Vanilla Cake:\t\t\t{float(e_vanilla.get()) * 400}\n\n')
        if e_blackforest.get() != '0':
            textReceipt.insert(
                END, f'Black Forest Cake :\t\t\t{float(e_blackforest.get()) * 550}\n\n')
        if e_brownie.get() != '0':
            textReceipt.insert(
                END, f'Brownie Cake:\t\t\t{float(e_brownie.get()) * 550}\n\n')
        if e_banana.get() != '0':
            textReceipt.insert(
                END, f'Banana Cake:\t\t\t{float(e_banana.get()) * 400}\n\n')
        if e_pineapple.get() != '0':
            textReceipt.insert(
                END, f'Pineapple Cake:\t\t\t{float(e_pineapple.get()) * 380}\n\n')
        if e_chocolate.get() != '0':
            textReceipt.insert(
                END, f'Chocolate Cake:\t\t\t{float(e_chocolate.get()) * 600}\n\n')
        if e_oreo.get() != '0':
            textReceipt.insert(
                END, f'Oreo Cheesecake:\t\t\t{float(e_oreo.get()) * 500}\n\n')
        textReceipt.insert(
            END, '********************************************************************\n')
...
cakeFrame = LabelFrame(menuFrame, bg='white', bd=10, text='Cakes',
                       fg='red4', font=('arial', 19, 'bold'), relief=RIDGE)
...
def send():
    ...
    def btn_click():
        ...
        if r:
            tkinter.messagebox.showinfo(
                "Send Success", "Message successfully sent!")
        else:
    ...
    messagelabel = Label(root2, text='Bill Details', font=(
        'arial', 18, 'bold underline'), bg='red4', fg='white')
    ...
    textMsg.insert(END, 'Receipt Ref:\t\t' + receipt_ref.get() +
                   '\t\t' + dateofOrder.get() + '\n\n')
...
Roti = Checkbutton(foodFrame, text='Roti', variable=var19, onvalue=1, offvalue=0,
                   font=('arial', 18, 'bold'), bg='white', command=roti).grid(row=0, sticky=W)

Daal = Checkbutton(foodFrame, text='Daal', variable=var20, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=daal).grid(row=1, sticky=W)
Fish = Checkbutton(foodFrame, text='Fish', variable=var21, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=fish).grid(row=2, sticky=W)
Sabji = Checkbutton(foodFrame, text='Sabji', variable=var22, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=sabji).grid(row=3, sticky=W)
Kebab = Checkbutton(foodFrame, text='Kebab', variable=var23, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=kebab).grid(row=4, sticky=W)
Chawal = Checkbutton(foodFrame, text='Chawal', variable=var24, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=chawal).grid(row=5, sticky=W)

Mutton = Checkbutton(foodFrame, text='Mutton', variable=var25, onvalue=1, offvalue=0,
                     font=('arial', 18, 'bold'), bg='white', command=mutton).grid(row=6, sticky=W)

Paneer = Checkbutton(foodFrame, text='Paneer', variable=var26, onvalue=1, offvalue=0,
                     font=('arial', 18, 'bold'), bg='white', command=paneer).grid(row=7, sticky=W)

Chicken = Checkbutton(foodFrame, text='Chicken', variable=var27, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=chicken).grid(row=8, sticky=W)
...
lassi = Checkbutton(drinksFrame, text='Lassi', variable=var1, onvalue=1, offvalue=0,
                    font=('arial', 18, 'bold'), bg='white', command=lassi).grid(row=0, sticky=W)

coffee = Checkbutton(drinksFrame, text='Coffee', variable=var2, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=coffee).grid(row=1, sticky=W)
faluda = Checkbutton(drinksFrame, text='Faluda', variable=var3, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=faluda).grid(row=2, sticky=W)
shikanji = Checkbutton(drinksFrame, text='Shikanji', variable=var4, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=shikanji).grid(row=3, sticky=W)
jaljeera = Checkbutton(drinksFrame, text='Jal-jeera', variable=var5, onvalue=1, offvalue=0,
                       font=('arial', 18, 'bold'), bg='white', command=jaljeera).grid(row=4, sticky=W)
roohafza = Checkbutton(drinksFrame, text='Rooh Afza', variable=var6, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=roohafza).grid(row=5, sticky=W)

masalachai = Checkbutton(drinksFrame, text='Masala Tea', variable=var7, onvalue=1, offvalue=0,
                         font=('arial', 18, 'bold'), bg='white', command=masalachai).grid(row=6, sticky=W)

badammilk = Checkbutton(drinksFrame, text='Badam Milk', variable=var8, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=badammilk).grid(row=7, sticky=W)
Coldrinks = Checkbutton(drinksFrame, text='Cold Drinks', variable=var9, onvalue=1, offvalue=0,
                        font=('arial', 18, 'bold'), bg='white', command=coldrinks).grid(row=8, sticky=W)
...
kitkatcake = Checkbutton(cakeFrame, text='Kitkat ', variable=var10, onvalue=1, offvalue=0,
                         font=('arial', 18, 'bold'), bg='white', command=kitkat).grid(row=2, sticky=W)
applecake = Checkbutton(cakeFrame, text='Apple ', variable=var11, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=apple).grid(row=1, sticky=W)
vanillacake = Checkbutton(cakeFrame, text='Vanilla ', variable=var12, onvalue=1, offvalue=0,
                          font=('arial', 18, 'bold'), bg='white', command=vanilla).grid(row=3, sticky=W)
blackforest = Checkbutton(cakeFrame, text='Black Forest', variable=var13, onvalue=1, offvalue=0,
                          font=('arial', 18, 'bold'), bg='white', command=blackforest).grid(row=8, sticky=W)
bananacake = Checkbutton(cakeFrame, text='Banana ', variable=var14, onvalue=1, offvalue=0,
                         font=('arial', 18, 'bold'), bg='white', command=banana).grid(row=4, sticky=W)
brownie = Checkbutton(cakeFrame, text='Brownie ', variable=var15, onvalue=1, offvalue=0, font=(
    'arial', 18, 'bold'), bg='white', command=brownie).grid(row=5, sticky=W)
pineapplecake = Checkbutton(cakeFrame, text='Pineapple ', variable=var16, onvalue=1, offvalue=0,
                            font=('arial', 18, 'bold'), bg='white', command=pineapple).grid(row=6, sticky=W)
chocolatecake = Checkbutton(cakeFrame, text='Chocolate', variable=var17, onvalue=1, offvalue=0,
                            font=('arial', 18, 'bold'), bg='white', command=chocolate).grid(row=7, sticky=W)

oreocheesecake = Checkbutton(cakeFrame, text='Oreo ', variable=var18, onvalue=1, offvalue=0,
                             font=('arial', 18, 'bold'), bg='white', command=oreo).grid(row=0, sticky=W)
...
textReceipt = Text(receiptFrame, width=46, height=14,
...
buttonTotal = Button(buttonFrame, padx=16, pady=1, bd=4, fg='white', font=(
    'arial', 14, 'bold'), width=4, text='Total', bg='red4', command=CostofItem).grid(row=0, column=0)
buttonReceipt = Button(buttonFrame, padx=16, pady=1, bd=4, fg='white', font=('arial', 14, 'bold'), width=4,
                       text='Receipt', bg='red4', command=receipt).grid(row=0, column=1)

buttonSave = Button(buttonFrame, padx=16, pady=1, bd=4, fg='white', font=('arial', 14, 'bold'), width=4,
                    text='Save', bg='red4', command=save).grid(row=0, column=2)
buttonSend = Button(buttonFrame, padx=16, pady=1, bd=4, fg='white', font=('arial', 14, 'bold'), width=4,
                    text='Send', bg='red4', command=send).grid(row=0, column=3)

buttonReset = Button(buttonFrame, padx=16, pady=1, bd=4, fg='white', font=(
    'arial', 14, 'bold'), width=4, text='Reset', bg='red4', command=reset).grid(row=0, column=4)
...
button7 = Button(calculatorFrame, padx=16, pady=1, bd=7, fg='yellow', font=(
    'arial', 16, 'bold'), width=4, text='7', bg='red4', command=lambda: btnClick(7)).grid(row=2, column=0)

button8 = Button(calculatorFrame, padx=16, pady=1, bd=7, fg='yellow', font=(
    'arial', 16, 'bold'), width=4, text='8', bg='red4', command=lambda: btnClick(8)).grid(row=2, column=1)

button9 = Button(calculatorFrame, padx=16, pady=1, bd=7, fg='yellow', font=(
    'arial', 16, 'bold'), width=4, text='9', bg='red4', command=lambda: btnClick(9)).grid(row=2, column=2)

buttonAdd = Button(calculatorFrame, padx=16, pady=1, bd=7, fg='yellow', font=(
    'arial', 16, 'bold'), width=4, text='+', bg='red4', command=lambda: btnClick('+')).grid(row=2, column=3)

button4 = Button(calculatorFrame, padx=16, pady=1, bd=7, fg='yellow', font=(
    'arial', 16, 'bold'), width=4, text='4', bg='red4', command=lambda: btnClick(4)).grid(row=3, column=0)

button5 = Button(calculatorFrame, padx=16, pady=1, bd=7, bg='white', font=(
    'arial', 16, 'bold'), width=4, text='5', fg='red4', command=lambda: btnClick(5)).grid(row=3, column=1)

button6 = Button(calculatorFrame, padx=16, pady=1, bd=7, bg='white', font=(
    'arial', 16, 'bold'), width=4, text='6', fg='red4', command=lambda: btnClick(6)).grid(row=3, column=2)

buttonsub = Button(calculatorFrame, padx=16, pady=1, bd=7, fg='yellow', font=(
    'arial', 16, 'bold'), width=4, text='-', bg='red4', command=lambda: btnClick('-')).grid(row=3, column=3)

button1 = Button(calculatorFrame, padx=16, pady=1, bd=7, fg='yellow', font=(
    'arial', 16, 'bold'), width=4, text='1', bg='red4', command=lambda: btnClick(1)).grid(row=4, column=0)

button2 = Button(calculatorFrame, padx=16, pady=1, bd=7, bg='white', font=(
    'arial', 16, 'bold'), width=4, text='2', fg='red4', command=lambda: btnClick(2)).grid(row=4, column=1)

button3 = Button(calculatorFrame, padx=16, pady=1, bd=7, bg='white', font=(
    'arial', 16, 'bold'), width=4, text='3', fg='red4', command=lambda: btnClick(3)).grid(row=4, column=2)

buttonmult = Button(calculatorFrame, padx=16, pady=1, bd=7, fg='yellow', font=(
    'arial', 16, 'bold'), width=4, text='*', bg='red4', command=lambda: btnClick('*')).grid(row=4, column=3)

buttonans = Button(calculatorFrame, padx=16, pady=1, bd=7, fg='yellow', font=(
    'arial', 16, 'bold'), width=4, text='Ans', bg='red4', command=btnAnswer).grid(row=5, column=0)

buttonclear = Button(calculatorFrame, padx=16, pady=1, bd=7, fg='yellow', font=('arial', 16, 'bold'), width=4,
                     text='Clear', bg='red4', command=btnClear).grid(row=5, column=1)

button0 = Button(calculatorFrame, padx=16, pady=1, bd=7, fg='yellow', font=(
    'arial', 16, 'bold'), width=4, text='0', bg='red4', command=lambda: btnClick(0)).grid(row=5, column=2)

buttondiv = Button(calculatorFrame, padx=16, pady=1, bd=7, fg='yellow', font=(
    'arial', 16, 'bold'), width=4, text='/', bg='red4', command=lambda: btnClick('/')).grid(row=5, column=3)
